...
// ─── Twilio SMS ───────────────────────────────────────────────────────────────
async function sendSMS(to, code, purposeLabel) {
...
function buildEmailHTML(code, purposeLabel, expiryMinutes = 10) {
    const digits = code.split('');
    const digitBoxes = digits.map(d => `<td style="padding:0 6px;">
...
async function sendEmail(to, code, purposeLabel) {
...
// ─── Public: Send OTP ─────────────────────────────────────────────────────────
async function sendOTP(identifier, type, purpose) {
    // Rate limit: max 3 OTPs per 10 minutes per identifier+purpose
    const since = new Date(Date.now() - 10 * 60 * 1000);
    const recentCount = await OTP_1.OTP.countDocuments({ identifier, purpose, createdAt: { $gte: since } });
    if (recentCount >= 3) {
    ...
    await OTP_1.OTP.deleteMany({ identifier, purpose, verified: false });
    const code = generateCode();
    const expiresAt = new Date(Date.now() + 10 * 60 * 1000);
    await OTP_1.OTP.create({ identifier, type, purpose, code, expiresAt });
    logger_1.logger.info(`[OTP SERVICE] OTP generated for ${identifier} (${purpose})`); // code never logged
    const purposeLabel = purpose === 'reset_password' ? 'reset your password' :
...
async function verifyOTP(identifier, code, purpose) {
    ...
    if (otp.code !== code && !isDevFallback) {
        await OTP_1.OTP.updateOne({ _id: otp._id }, { $inc: { attempts: 1 } });
        const remaining = 4 - otp.attempts;
        return { success: false, message: `Incorrect OTP. ${remaining} attempt(s) remaining.` };
    }
    await OTP_1.OTP.updateOne({ _id: otp._id }, { verified: true });
